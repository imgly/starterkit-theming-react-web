/**
 * Theming Editor Plugin - Complete Theming Configuration for CE.SDK
 *
 * This plugin provides a production-ready theming editor configuration with
 * templates, elements, asset libraries, and comprehensive editing capabilities.
 *
 * @example Basic usage
 * ```typescript
 * import CreativeEditorSDK from '@cesdk/cesdk-js';
 * import { ThemingEditorConfig } from './plugin';
 *
 * const cesdk = await CreativeEditorSDK.create('#editor', config);
 * await cesdk.addPlugin(new ThemingEditorConfig());
...
/**
 * Theming Editor configuration plugin.
 *
 * Provides a complete theming experience optimized for creating
 * themed designs with customizable styles and branding.
 *
 * @public
 */
export class ThemingEditorConfig implements EditorPlugin {
  /**
   * Unique identifier for this plugin.
   * Used to identify the plugin in the CE.SDK plugin registry.
   */
  name = 'cesdk-theming-editor';
  ...
  /**
   * Initialize the theming editor configuration.
   *
  ...
  async initialize({ cesdk, engine }: EditorPluginContext) {
    if (cesdk) {
      ...
      // This ensures a clean slate when applying the theming editor config
      cesdk.resetEditor();
      // #endregion
